...
def parse_args(argv: list[str]):
    ap = argparse.ArgumentParser(
        description="Build intelligence artifacts from aggregates CSV"
    )
    ...
    ap.add_argument(
        "--top-cells", type=int, default=200, help="Top scoring cells used for hubs"
    )
...
def main(argv: list[str]):
    ...
    if "score" not in df_high.columns:
        ...
        df_high["_uniq_factor"] = (
            df_high["unique_trips"] / df_high["point_count"].clip(lower=1)
        ).clip(0, 1)
        df_high["score"] = (
            0.6 * df_high["_trip_intensity"] + 0.4 * df_high["_uniq_factor"]
        )
    ...
    builder.build_stop_clusters(
        df_high, out_dir / "stop_clusters.csv", top_n=args.top_cells
    )
